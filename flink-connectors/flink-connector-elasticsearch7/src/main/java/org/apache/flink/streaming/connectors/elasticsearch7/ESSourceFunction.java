...
/** */
public class ESSourceFunction extends RichParallelSourceFunction<RowData>
    ...
    public void run(SourceContext<RowData> ctx) throws Exception {
        ...
        logger.info(
                "subTask:{} es source function current task index:{} parallelNum:{}",
                subTaskIndex,
                subTaskIndex,
                parallelNum);
        // 数据源获取
        try {
            logger.info(
                    "subTask:{} step1:es source function start execute hosts:{} index:{} document_type:{} fetch_size:{}",
                    subTaskIndex,
                    hosts,
                    index,
                    document_type,
            ...
            logger.info(
                    "subTask:{} step2:es source function put settings set max_result_window success ",
                    subTaskIndex);

            // 初始化分页
            initPage();
            logger.info(
                    "subTask:{} step3:es source function get total:{} totalPage:{}",
                    subTaskIndex,
                    total,
                    totalPage);
            int from = 0;
            ...
            logger.info(
                    "subTask:{} step4:es source function execute query start from:{} size:{} ",
                    subTaskIndex,
                    from,
                    size);
            ...
            for (int page = 0; page < totalPage; page++) {
                ...
                if (!isRunning) {
                    logger.info(
                            "subTask:{} es source function query now is not running break while",
                            subTaskIndex);
                    cancel();
                    break;
                }
                ...
                logger.info(
                        "subTask:{} es source function query current page:{} from:{} size:{}",
                        subTaskIndex,
                        page,
                        from,
                        size);
                ...
                try {
                    ...
                    // 请求报错 间隔3秒重试
                    logger.error(
                            "subTask:{} es source function query request param:{} have error:{}",
                            subTaskIndex,
                            queryJson,
                            e);
                    Thread.sleep(3000);
                    logger.error(
                            "subTask:{} es source function query sleep 3 s again request param:{}",
                            subTaskIndex,
                            queryJson);
                    response = client.performRequest(request);
                }
                ...
                if (searchList.size() <= 0) {
                    logger.info(
                            "subTask:{} es source function query result size less than 0 exit query current from:{} page:{}",
                            subTaskIndex,
                            from,
                            page);
                }
                        ...
                        .forEach(
                                sourceAsString -> {
                                    if (StringUtils.isBlank(sourceAsString)) {
                                        logger.info(
                                                "subTask:{} es source function query row is empty:{}",
                                                subTaskIndex);
                                    }
                                    Map resultMap =
                                            JSONObject.parseObject(sourceAsString, Map.class);
                                    String source =
                                            JSONObject.toJSONString(resultMap.get("_source"));
                                    try {
                                        ctx.collect(deserializer.deserialize(source.getBytes()));
                                    } catch (IOException e) {
                                        logger.error(
                                                "subTask:{} error s source function query ctx collect data:{} have error:{}",
                                                subTaskIndex,
                                                sourceAsString,
            ...
            logger.info(
                    "subTask:{} step5:es source function execute query end startTime:{} endTime:{}",
                    subTaskIndex,
                    startTime,
                    endTime);
        } catch (Exception e) {
            logger.error(
                    "subTask:{} error es source function query have error:{}",
                    subTaskIndex,
                    ExceptionUtils.getStackTrace(e));
            throw new RuntimeException(ExceptionUtils.getStackTrace(e));
        } finally {
            logger.info(
                    "subTask:{} step6:es source function query end read cancel client:{}..... ",
                    subTaskIndex,
                    client);
            cancel();
            logger.info(
                    "subTask:{} step6:es source function query cancel client success.",
                    subTaskIndex);
        }
    }
    ...
    public void cancel() {
        ...
        try {
            ...
            if (client != null) {
                ...
                logger.info(
                        "subTask:{} es source function query cancel client but client is null.",
                        subTaskIndex);
            }
            logger.info(
                    "subTask:{} step6:es source function query cancel client success.",
            ...
            logger.error(
                    "subTask:{} error es source function cancel client have error:{}",
    ...
    public void initClient() throws Exception {
        ...
        logger.info(
                "subTask:" + subTaskIndex + " step2:es source function init client success ",
                subTaskIndex);
    }
    ...
    public void close() throws Exception {
        ...
        logger.info(
                "subTask:{} step6:es source function query close client success.", subTaskIndex);
    }
}
